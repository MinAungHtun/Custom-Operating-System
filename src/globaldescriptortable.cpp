...
namespace MYYOS
{
    ...
    GlobalDescriptorTable::GlobalDescriptorTable(const multiboot_info& multibootHeader)
        : nullSegmentSelector(0, 0, 0),
            ...
            codeSegmentSelector(0, 1024*multibootHeader.mem_upper, 0x9A),
            dataSegmentSelector(0, 1024*multibootHeader.mem_upper, 0x92),
            taskStateSegmentSelector(0, 1024*multibootHeader.mem_upper, 0)
    {
        uint32_t i[2];
        i[1] = (uint32_t)this;
    ...
    GlobalDescriptorTable::SegmentSelector::SegmentSelector(uint32_t base, uint32_t limit, uint8_t type)
    {
        ...
        if (limit <= 65536)
        {
            ...
            // 64K of memory should be enough 
            target[6] = 0x40;
        }
        else
        {
            ...
            // This is done by discarding the 12 least significant bits, but this is only legal, if they are all ==1, so they are implicitly still there
